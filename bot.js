...
async function whatsAsena () {
    ...
    conn.on ('credentials-updated', async () => {
        console.log(
            chalk.blueBright.italic('✅ WIPER-X Login Information Updated!')
        );
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('open', async () => {
        console.log(
            chalk.green.bold('✅ WIPER-X Login successful!')
        );

        console.log(
            chalk.blueBright.italic('⬇️ WIPER-X Installing External Plugins...')
        );
        ...
        console.log(
            chalk.blueBright.italic('⬇️ WIPER-X Installing Plugins...')
        );
        ...
        console.log(
            chalk.green.bold('✅ WIPER-X Plugins Installed!')
        );
        ...
        if (config.WORKTYPE == 'public') {
            ...
            else { await conn.sendMessage(conn.user.jid, '*whiper-x Working as Private! 💥*\n\n_Please do not try plugins here. This is your LOG number._\n_You can try commands to any chat :)_\n\n*Your bot working as private. To change it, use* _.setvar WORK_TYPE:public_\n\n*Thanks for using WIPER-X 💌*', MessageType.text);
                await git.fetch();
                var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
                if (commits.total === 0) {
                    await conn.sendMessage(
                        ...
                        try {
                            ...
                            if (error.message.includes('429')) return;
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ HATA ÇÖZÜMLEME [💥WIPER-X💥] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [💥WIPER-X💥] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
